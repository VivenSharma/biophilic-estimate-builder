...
function p1Table(doc, data, topY){
  ...
  // ── Total band ──
  doc.save().fillColor(C.darkGreen).rect(M, y, CW, 36).fill().restore();
  doc.font("Helvetica-BoldOblique").fontSize(7.5).fillColor(C.white)
     .text(amountInWords(total), M+8, y+6, {width:CW*0.6, lineBreak:false});
  doc.font("Helvetica-Bold").fontSize(9).fillColor(C.white)
     .text("TOTAL ESTIMATE", M+8, y+21, {lineBreak:false});
  doc.font("Helvetica-Bold").fontSize(14).fillColor(C.lightGreen)
     .text("Rs. "+rupee(total), M, y+9, {width:CW-8, align:"right", lineBreak:false});
  y += 36;
...
function p2HeaderBand(doc, data){
  doc.font("Helvetica-Bold").fontSize(11).fillColor(C.darkGreen)
     .text(data.companyName||"", M, 18, {lineBreak:false});
  const ref = `Estimate Ref: ${data.projectRef||""}  |  ${data.clientName||""}`;
  doc.font("Helvetica").fontSize(7.5).fillColor(C.textGray)
     .text(ref, M, 20, {width:CW, align:"right", lineBreak:false});
  hline(doc, M, 36, CW);
}
